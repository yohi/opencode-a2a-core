...
async *run(pluginId, message, { abortSignal, contextId }) {
  ...
  if (!plugin) throw new NonRetriableError("PluginNotFound")
...
- タイミング攻撃対策: 受信トークンと保存トークンをそれぞれ SHA-256 でハッシュして固定長 `Buffer`（32 バイト）にしてから `crypto.timingSafeEqual` で比較する。これにより入力長によらず常に固定長バッファ同士の比較となり、`crypto.timingSafeEqual` が長さ不一致時に投げる `RangeError`（500 として漏れる）も、入力長自体のタイミング漏洩も両方回避できる。
...
- Node 22 LTS 固定。
- `corepack prepare pnpm@9 --activate` で pnpm のメジャーバージョンを固定し、`package.json` の `"packageManager": "pnpm@9"` と一致させる（グローバル汚染を避けつつ再現性を担保）。
