...
### PR ベース規則（Stacked PR の純粋差分保証）
- **派生元 = Phase Base のタスク**: `gh pr create --base phase/<N>-<short> --head task/<...>` で作成。
- **派生元 = 直前 Task のタスク**: `gh pr create --base task/<前タスクのブランチ名> --head task/<...>` で作成。これにより GitHub 上で当該タスクの純粋差分のみがレビュー対象となる。
- 直前タスクが Phase Base にマージされた段階で、後続タスク PR の base を順次 `phase/<N>-<short>` へ rebase（`gh pr edit --base ...` または `git rebase --onto`）して付け替える。

### Phase ベース集約
- 各 Phase は **Phase Base ブランチ**（例: `phase/1-client`, `phase/2-server`）を持ち、最終的に全 Task が Phase Base に集約される。
...
- **各 Task の最終アクションとして Draft PR を作成する**（base は `PR ベース規則` に従う）:
  ```bash
  # Base 派生タスク: base は Phase Base
  gh pr create --base phase/<N>-<short> --head task/<...> --draft \
    --title "<conventional commit title>" --body-file .github/PR_BODY.md
  # 直前 Task 派生タスク: base は直前 Task のブランチ
  gh pr create --base task/<前タスク> --head task/<...> --draft \
    --title "<conventional commit title>" --body-file .github/PR_BODY.md
  ```
...
- **最終アクション**:
  - [ ] `gh pr create --base task/1-1-auth-provider-factory --head task/1-2-client-manager-di --draft`
  - [ ] Task 1.1 が `phase/1-client` にマージされたら `gh pr edit task/1-2-client-manager-di --base phase/1-client` で base を付け替える
...
- **最終アクション**:
  - [ ] `gh pr create --base task/1-2-client-manager-di --head task/1-3-config-wiring --draft`
  - [ ] Task 1.2 が `phase/1-client` にマージされたら `gh pr edit task/1-3-config-wiring --base phase/1-client` で base を付け替える
...
- **最終アクション**:
  - [ ] `gh pr create --base task/2-1-coder-agent-definition --head task/2-3-thin-router --draft`
  - [ ] Task 2.1 が `phase/2-server` にマージされたら `gh pr edit task/2-3-thin-router --base phase/2-server` で base を付け替える
...
- **最終アクション**:
  - [ ] `gh pr create --base task/2-3-thin-router --head task/2-4-cancellation-bridge --draft`
  - [ ] Task 2.3 が `phase/2-server` にマージされたら `gh pr edit task/2-4-cancellation-bridge --base phase/2-server` で base を付け替える
...
- **確認手順** (Devcontainer 内):
  ...
  - [ ] `pnpm lint && pnpm typecheck`
- **最終アクション**:
  - [ ] `gh pr create --base task/2-4-cancellation-bridge --head task/2-5-remove-taskwrapper --draft`
  - [ ] Task 2.4 が `phase/2-server` にマージされたら `gh pr edit task/2-5-remove-taskwrapper --base phase/2-server` で base を付け替える
...
- **実装内容**:
  - [ ] `pnpm -F @google/gemini-cli-a2a-server test` の Devcontainer 内実行ログを残す
  - [ ] `pnpm -F @google/gemini-cli-core test -- src/agents/a2a-client-manager.test.ts` の実行ログを残す
  - [ ] 必要なドキュメント更新（`README.md` の旧 `TaskWrapper` 記述削除など）
- **確認手順** (Devcontainer 内):
  - [ ] 上記 2 コマンドが Green
  - [ ] `pnpm test:coverage` でカバレッジ閾値維持
- **最終アクション**:
  - [ ] `gh pr create --base task/2-5-remove-taskwrapper --head task/2-6-final-verification --draft`
  - [ ] Task 2.5 が `phase/2-server` にマージされたら `gh pr edit task/2-6-final-verification --base phase/2-server` で base を付け替える
