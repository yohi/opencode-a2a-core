...
describe('JSON_RPC_ERRORS', () => {
  it('has all required error codes', () => {
    ...
    expect(JSON_RPC_ERRORS.CANCEL_TIMEOUT).toBe(-32003);
    expect(JSON_RPC_ERRORS.TASK_ALREADY_COMPLETED).toBe(-32004);
    expect(JSON_RPC_ERRORS.TASK_ALREADY_FAILED).toBe(-32005);
  });
});
