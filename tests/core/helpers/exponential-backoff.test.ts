...
describe('computeBackoffMs', () => {
  ...
  it('validates input options', () => {
    expect(() => computeBackoffMs(1, { ...opts, initialMs: 0 })).toThrow(
      'initialMs must be > 0'
    );
    expect(() => computeBackoffMs(1, { ...opts, multiplier: -1 })).toThrow(
      'multiplier must be >= 0'
    );
    expect(() => computeBackoffMs(1, { ...opts, jitterRatio: 1.1 })).toThrow(
      'jitterRatio must be between 0 and 1'
    );
    expect(() => computeBackoffMs(1, { ...opts, jitterRatio: -0.1 })).toThrow(
      'jitterRatio must be between 0 and 1'
    );
  });
