...
describe('TaskRunner — happy path', () => {
  ...
  it("filters out reserved 'task' chunks emitted by plugin and logs warning", async () => {
    ...
    registry.register(
      mkPlugin('bad-plugin', async function* () {
        ...
        yield {
          ...
          task: {
            id: 'fake-id',
            status: { state: 'TASK_STATE_PENDING', timestamp: '' },
          },
    ...
    const ctl = new AbortController();
    const out = await drain(
      runner.run('bad-plugin', mkMessage(), { abortSignal: ctl.signal })
    );
  ...
  it('failure in plugin yields task → WORKING → FAILED and persists it', async () => {
    ...
    const runPromise = (async () => {
      for await (const chunk of runner.run('p', mkMessage(), {
    ...
    expect(task.statusHistory?.[task.statusHistory.length - 1].state).toBe(
      'TASK_STATE_FAILED'
    );
  });
  ...
  it('does not overwrite terminal/special state set by plugin', async () => {
    ...
    const statusUpdates = out.filter((c) => c.kind === 'status-update') as {
      status: { state: string };
    }[];
  ...
  it("handles 'message' chunks and persists them to history", async () => {
    ...
    expect(persisted?.history?.[0].parts[0]).toEqual({
      kind: 'text',
...
describe('TaskRunner — retries and errors', () => {
  ...
  it('does NOT retry if NonRetriableError is thrown', async () => {
    ...
    const ctl = new AbortController();
    await expect(
      drain(runner.run('no-retry', mkMessage(), { abortSignal: ctl.signal }))
  ...
  it('aborts sleep when AbortSignal is triggered and sets CANCELED status', async () => {
    ...
    const runPromise = drain(
      runner.run('long-retry', mkMessage(), { abortSignal: ctl.signal })
    );
...
describe('TaskRunner — all attempts fail', () => {
  it('after maxAttempts fails, emits FAILED with error in status.message', async () => {
    ...
    const runPromise = (async () => {
      for await (const chunk of runner.run('always-fail', mkMessage(), {
        abortSignal: ctl.signal,
      })) {
        out.push(chunk);
      }
...
describe('TaskRunner — cancellation mid-stream', () => {
  it('when abort fires while plugin is yielding, run terminates with CANCELED', async () => {
    ...
    const out: StreamResponse[] = [];
    for await (const chunk of runner.run('slow', mkMessage(), {
      abortSignal: ctl.signal,
    })) {
      out.push(chunk);
...
describe('TaskRunner — post-yield error does not retry', () => {
  it('failure after first yield triggers FAILED without retry', async () => {
    ...
    const out: StreamResponse[] = [];
    const runPromise = (async () => {
      for await (const chunk of runner.run('yield-then-fail', mkMessage(), {
        abortSignal: ctl.signal,
      })) {
        out.push(chunk);
      }
...
describe('TaskRunner — non-retriable errors', () => {
  it('emits FAILED without retry when plugin throws NonRetriableError', async () => {
    ...
    const out: StreamResponse[] = [];
    const runPromise = (async () => {
      for await (const chunk of runner.run('permanent', mkMessage(), {
        abortSignal: ctl.signal,
      })) {
        out.push(chunk);
      }
  ...
  it('throws error immediately if maxAttempts is 0 or less', async () => {
    ...
    const ctl = new AbortController();
    await expect(
      drain(runner.run('any', mkMessage(), { abortSignal: ctl.signal }))
