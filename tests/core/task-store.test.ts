...
describe('InMemoryTaskStore', () => {
  ...
  it('updateStatus protects against input mutation', async () => {
    ...
    const assertMessageIntegrity = (
      msg: { parts: { kind: string; text?: string }[] } | undefined
    ) => {
  ...
  it('appendStreamChunk throws on unhandled kinds', async () => {
    ...
    await expect(store.appendStreamChunk(t.id, chunk)).rejects.toThrow(
      /Unhandled stream chunk kind "unknown-kind"/
    );
  });
